...
/*      */         }
/*      */ 
/*  746 */         effect.applyTo(c.getPlayer(), pos);
/*      */       }
/*      */     }
/*      */   }
/*      */ 
...
/*  754 */       return;
/*      */     }
/*  756 */     if ((chr.hasBlockedInventory()) || (chr.getMap() == null)) {
/*  757 */       return;
/*      */     }
/*  759 */     AttackInfo attack = DamageParse.parseDmgM(slea, chr);
/*  760 */     if (attack == null) {
/*  761 */       c.getSession().write(CWvsContext.enableActions());
/*  762 */       return;
/*      */     }
/*  764 */     boolean mirror = chr.getBuffedValue(MapleBuffStat.SHADOWPARTNER) != null;
/*  765 */     double maxdamage = chr.getStat().getCurrentMaxBaseDamage();
/*  766 */     Item shield = c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem((byte)-10);
/*  767 */     int attackCount = (shield != null) && (shield.getItemId() / 10000 == 134) ? 2 : 1;
/*  768 */     int skillLevel = 0;
/*  769 */     MapleStatEffect effect = null;
/*  770 */     Skill skill = null;
/*      */ 
/*  772 */     if (attack.skill != 0) {
/*  773 */       skill = SkillFactory.getSkill(GameConstants.getLinkedAranSkill(attack.skill));
/*  774 */       if ((skill == null) || ((GameConstants.isAngel(attack.skill)) && (chr.getStat().equippedSummon % 10000 != attack.skill % 10000))) {
/*  775 */         c.getSession().write(CWvsContext.enableActions());
/*  776 */         return;
/*      */       }
/*  778 */       skillLevel = chr.getTotalSkillLevel(skill);
/*  779 */       effect = attack.getAttackEffect(chr, skillLevel, skill);
/*  780 */       if (effect == null) {
/*  781 */         return;
/*      */       }
/*  783 */       if (GameConstants.isEventMap(chr.getMapId())) {
/*  784 */         for (MapleEventType t : MapleEventType.values()) {
/*  785 */           MapleEvent e = ChannelServer.getInstance(chr.getClient().getChannel()).getEvent(t);
/*  786 */           if ((e.isRunning()) && (!chr.isGM())) {
/*  787 */             for (int i : e.getType().mapids) {
/*  788 */               if (chr.getMapId() == i) {
/*  789 */                 chr.dropMessage(5, "You may not use that here.");
/*  790 */                 return;
/*      */               }
/*      */             }
/*      */           }
/*      */         }
/*      */       }
/*  796 */       maxdamage *= (effect.getDamage() + chr.getStat().getDamageIncrease(attack.skill)) / 100.0D;
/*  797 */       attackCount = effect.getAttackCount();
/*      */ 
/*  799 */       if ((effect.getCooldown(chr) > 0) && (!chr.isGM()) && (!energy)) {
/*  800 */         if (chr.skillisCooling(attack.skill)) {
/*  801 */           c.getSession().write(CWvsContext.enableActions());
/*  802 */           return;
/*      */         }
/*  804 */         c.getSession().write(CField.skillCooldown(attack.skill, effect.getCooldown(chr)));
/*  805 */         chr.addCooldown(attack.skill, System.currentTimeMillis(), effect.getCooldown(chr) * 1000);
/*      */       }
/*      */     }
/*  808 */     attack = DamageParse.Modify_AttackCrit(attack, chr, 1, effect);
/*  809 */     attackCount *= (mirror ? 2 : 1);
/*  810 */     if (!energy) {
/*  811 */       if (((chr.getMapId() == 109060000) || (chr.getMapId() == 109060002) || (chr.getMapId() == 109060004)) && (attack.skill == 0)) {
/*  812 */         MapleSnowball.MapleSnowballs.hitSnowball(chr);
/*      */       }
/*      */ 
/*  815 */       int numFinisherOrbs = 0;
/*  816 */       Integer comboBuff = chr.getBuffedValue(MapleBuffStat.COMBO);
/*      */ 
/*  818 */       if (isFinisher(attack.skill) > 0) {
/*  819 */         if (comboBuff != null) {
/*  820 */           numFinisherOrbs = comboBuff.intValue() - 1;
/*      */         }
/*  822 */         if (numFinisherOrbs <= 0) {
/*  823 */           return;
/*      */         }
/*  825 */         chr.handleOrbconsume(isFinisher(attack.skill));
/*  826 */         if (!GameConstants.GMS) {
/*  827 */           maxdamage *= numFinisherOrbs;
/*      */         }
/*      */       }
/*      */     }
/*  831 */     chr.checkFollow();
/*  832 */     if (!chr.isHidden())
/*  833 */       chr.getMap().broadcastMessage(chr, CField.closeRangeAttack(chr.getId(), attack.tbyte, attack.skill, skillLevel, attack.display, attack.speed, attack.allDamage, energy, chr.getLevel(), chr.getStat().passive_mastery(), attack.unk, attack.charge), chr.getTruePosition());
/*      */     else {
/*  835 */       chr.getMap().broadcastGMMessage(chr, CField.closeRangeAttack(chr.getId(), attack.tbyte, attack.skill, skillLevel, attack.display, attack.speed, attack.allDamage, energy, chr.getLevel(), chr.getStat().passive_mastery(), attack.unk, attack.charge), false);
/*      */     }
/*  837 */     DamageParse.applyAttack(attack, skill, c.getPlayer(), attackCount, maxdamage, effect, mirror ? AttackType.NON_RANGED_WITH_MIRROR : AttackType.NON_RANGED);
/*      */   }
/*      */ 
